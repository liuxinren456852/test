...
public:
	...
	bool predict(double dt, const V3D& acc, const V3D& gyr, const V3D& gravity_feedback, bool update_jacobian_=true) {
		if (!isInitialized())
			return false;
		...
		if (update_jacobian_) {
			...
			Ft.block<3,3>(GlobalState::vel_,GlobalState::att_) = -state_tmp.qbn_.toRotationMatrix() * skew(un_acc);
			Ft.block<3,3>(GlobalState::vel_,GlobalState::acc_) = -state_tmp.qbn_.toRotationMatrix();
			
			Ft.block<3,3>(GlobalState::att_,GlobalState::att_) = -skew(un_gyr);
	...
	bool predict(double dt, const V3D& acc, const V3D& gyr, const Q4D& qbn, bool update_jacobian_=true) {
		if (!isInitialized())
			return false;
		...
		GlobalState state_tmp = state_;
		V3D un_acc_0 = (acc_last - state_tmp.ba_) + (qbn).normalized().inverse() * state_tmp.gn_;
		V3D un_gyr = 0.5 * (gyr_last + gyr) - state_tmp.bw_;
		Q4D dq = axis2Quat(un_gyr * dt);
		state_tmp.qbn_ =  (state_tmp.qbn_ * dq).normalized();
		V3D un_acc_1 = (acc - state_tmp.ba_) + (qbn).normalized().inverse() * state_tmp.gn_;
		V3D un_acc = 0.5 * (un_acc_0 + un_acc_1);
    ...
    static void calcRP(const V3D& acc, double& roll, double& pitch) {
	pitch = -sign(acc.z()) * asin(acc.x() / G0);
	roll = sign(acc.z()) * asin(acc.y() / G0);
    }
    
    
    void set(const GlobalState& state) {
	state_ = state;
    };
    
    void update(const GlobalState& state, const Eigen::Matrix<double, GlobalState::DIM_OF_STATE_, GlobalState::DIM_OF_STATE_>& covariance) {
	state_ = state;
    ...
    void initialization(double time, const V3D& rn, const V3D& vn, const Q4D& qbn, const V3D& ba, const V3D& bw, const V3D& acc, const V3D& gyr) {
	state_ = GlobalState(rn, vn, qbn, ba, bw);
	time_ = time;
	acc_last = acc;
	gyr_last = gyr;
    ...
    void initialization(double time, const V3D& rn, const V3D& vn, const V3D& ba, const V3D& bw, double roll=0.0, double pitch=0.0, double yaw=0.0) {
    ...
    void initialization(double time, const V3D& rn, const V3D& vn, const V3D& ba, const V3D& bw, const V3D& acc, const V3D& gyr, double roll=0.0, double pitch=0.0, double yaw=0.0) {
	state_ = GlobalState(rn, vn, rpy2Quat(V3D(roll, pitch, yaw)), ba, bw);
	time_ = time;
	acc_last = acc;
	gyr_last = gyr;
...
// 	double pba = pow(1000*ug,2);
// 	double pbg = pow(1000*dph,2);
	...
	// V3D(0.01,0.01,0.01).array().square();
	
// 	double peba = pow(0.2,2);
// 	double pebg = pow(0.02,2);
// 	double pweba = pow(0.0002,2);
// 	double pwebg = pow(2.0e-5,2);
...
// 	    V3D ba_cov = covariance_.block<3,3>(GlobalState::acc_,GlobalState::acc_).diagonal();
// 	    V3D bw_cov = covariance_.block<3,3>(GlobalState::gyr_,GlobalState::gyr_).diagonal();
// 			V3D vel_cov = covariance_.block<3,3>(GlobalState::vel_,GlobalState::vel_).diagonal();
// 
// 	    covariance_.setZero();
// 	    covariance_.block<3,3>(GlobalState::pos_,GlobalState::pos_) = covPos.asDiagonal(); 		//pos
// 	    covariance_.block<3,3>(GlobalState::vel_,GlobalState::vel_) = vel_cov.asDiagonal(); 		//vel
// 	    covariance_.block<3,3>(GlobalState::att_,GlobalState::att_) = V3D(covRoll, covPitch, covYaw).asDiagonal();	//att
// 	    covariance_.block<3,3>(GlobalState::acc_,GlobalState::acc_) = ba_cov.asDiagonal();				//ba
// 	    covariance_.block<3,3>(GlobalState::gyr_,GlobalState::gyr_) = bw_cov.asDiagonal();				//bg
// 	    cout << "ba_cov: " << ba_cov.transpose() << endl;
    ...
    Eigen::Matrix<double, GlobalState::DIM_OF_NOISE_, GlobalState::DIM_OF_NOISE_> noise_;
