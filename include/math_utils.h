...
// template <typename Derived>
// static Eigen::Matrix<typename Derived::Scalar, 3, 3> Rinvleft(const Eigen::MatrixBase<Derived> q)
// {
//     typedef typename Derived::Scalar Scalar_t;
// 
//     Eigen::Matrix<Scalar_t, 3, 3> ans;
//     Eigen::Matrix<Scalar_t, 3, 1> axis = q;
//     Scalar_t half_theta = axis.norm()/static_cast<Scalar_t>(2.0);
//     Eigen::Matrix<Scalar_t, 3, 1> a = axis / axis.norm();
//     
//     Scalar_t cot_half_theta = cos(half_theta)/sin(half_theta);
//     Scalar_t s = half_theta*cot_half_theta;
//     ans = s*Eigen::Matrix<typename Derived::Scalar, 3, 3>::Identity() + (static_cast<Scalar_t>(1.0)-s)*a*a.transpose() - half_theta*skew(a);
// 
//     return ans;
// }
