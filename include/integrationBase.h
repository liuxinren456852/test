#ifndef _INTEGRATION_BASE_H_
#define _INTEGRATION_BASE_H_
  ...
  public:
	...
	result_delta_v = delta_v + un_acc * _dt;
	    ...
	    // the order of a and theta is exchanged. and F = I + F*dt + 0.5*F^2*dt^2
            ...
            F.block<3, 3>(GlobalState::pos_, GlobalState::att_) = -0.25 * delta_q.toRotationMatrix() * R_a_0_x * _dt * _dt + 
                                  -0.25 * result_delta_q.toRotationMatrix() * R_a_1_x * (Matrix3d::Identity() - R_w_x * _dt) * _dt * _dt;
            F.block<3, 3>(GlobalState::pos_, GlobalState::vel_) = MatrixXd::Identity(3,3) * _dt;
            F.block<3, 3>(GlobalState::pos_, GlobalState::acc_) = -0.25 * (delta_q.toRotationMatrix() + result_delta_q.toRotationMatrix()) * _dt * _dt;
            F.block<3, 3>(GlobalState::pos_, GlobalState::gyr_) = -0.25 * result_delta_q.toRotationMatrix() * R_a_1_x * _dt * _dt * -_dt;
            ...
            F.block<3, 3>(GlobalState::vel_, GlobalState::att_) = -0.5 * delta_q.toRotationMatrix() * R_a_0_x * _dt + 
                                  -0.5 * result_delta_q.toRotationMatrix() * R_a_1_x * (Matrix3d::Identity() - R_w_x * _dt) * _dt;
            F.block<3, 3>(GlobalState::vel_, GlobalState::vel_) = Matrix3d::Identity();
            F.block<3, 3>(GlobalState::vel_, GlobalState::acc_) = -0.5 * (delta_q.toRotationMatrix() + result_delta_q.toRotationMatrix()) * _dt;
            F.block<3, 3>(GlobalState::vel_, GlobalState::gyr_) = -0.5 * result_delta_q.toRotationMatrix() * R_a_1_x * _dt * -_dt;
	...
	Vector3d dv = vk1 + 0.5*ak1.cross(vk1) + 1.0/12*(ak0.cross(vk1) + vk0.cross(ak1));
	Vector3d da = 0.5*(ak0+ak1);
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        midPointIntegration(_dt, acc_0, gyr_0, acc_1, _gyr_1, delta_p, delta_q, delta_v,
